...
namespace MicroPipes
{
    public static partial class Extensions
    {
        public static T Unwrap<T>(this Option<T> option)
            => option.Match(p => p, () => throw new InvalidOperationException("Unwrap empty options"));

        public static bool TryGetValue<TKey, TValue>(this HashMap<TKey, TValue> map, TKey key, out TValue value)
        {
            var v = default(TValue);
            var result = map.Find(key).Match(p =>
            {
                v = p;
                return true;
            }, () => false);
